...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      const { title, description, steps, status, notionLink } = req.body;

      if (!title || !description || !notionLink) {
      ...
      const roadmapData = {
        ...
        status: status || 'draft',
        notionLink, // Store the file URL instead
    ...
    try {
      const { id, title, description, steps, status, notionLink } = req.body;
      ...
      const updatedData = {
        ...
        steps: Array.isArray(steps) ? steps : JSON.parse(steps || '[]'),
        status,
        notionLink, // Update the file URL
        updatedAt: admin.database.ServerValue.TIMESTAMP,
      };
